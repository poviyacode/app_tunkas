...
export default class PostsHomeComponent {
  ...
  constructor() {
    ...
    this.activatedRoute.paramMap.subscribe((params) => {
      ...
      this.order = params.get('order') || 'top';
  ...
  navigateTo(type: 'top' | 'recent'): void {
    if (type === 'top') {
      this.router.navigate(['/posts']);
      this.order = 'top';
    } else { // 'recent'
      this.router.navigate(['/posts'], { queryParams: { order: 'recent' } });
      this.order = 'recent';
    }
  ...
  isLinkActive(type: 'top' | 'recent'): boolean {
    const currentUrl = this.router.url;

    if (type === 'top') {
      // 'Top' está activo si la URL actual es exactamente /posts Y NO tiene el query param 'order=recent'.
      // Usamos router.isActive para verificar la ruta base y luego revisamos el query param.
      return this.router.isActive('/posts', { paths: 'exact', queryParams: 'ignored', fragment: 'ignored', matrixParams: 'ignored' }) &&
        !currentUrl.includes('order=recent');

    } else { // 'recent'
      // 'Recent' está activo si la URL actual es /posts E incluye el query param 'order=recent'.
      return currentUrl.includes('/posts?order=recent');
    }
  }
  ...
  headPage() {
    const data: MetaTag = {
      title: `Yuvinka :: Posts`,
      description: "The best platform to find posts.",
      path: ``,
  ...
  onPost(post: Post) {
    ...
    this.router.navigate(['posts', 'pu', post.slug]);
  ...
  buttonPrimaryClass() {
    return Tools.buttonPrimaryClass();
  }
